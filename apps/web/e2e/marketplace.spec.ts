...
test("browses compact courses from reusable marketplace navigation", async ({ page }) => {
  ...
  await expect(effectCard.locator('[data-slot="card-description"]')).toHaveCSS("font-family", /Hind/);
  ...
  await expect(page.locator("body")).toHaveCSS("font-family", /Hind/);
  ...
  await expect(page.getByText("Master Effect through 40 hands-on katas.", { exact: true })).toHaveCSS("font-family", /Hind/);
