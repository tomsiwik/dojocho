...
import { Presence as PresenceNS } from 'radix-ui/internal'
import { usePathname } from 'fumadocs-core/framework'

const Presence = PresenceNS.Presence
 ...
 *
 * Lightweight replacement for `scroll-into-view-if-needed` using the native API
 * (the original lib is not a dep of this app).
 */
export function useAutoScroll(active: boolean, ref: RefObject<HTMLElement | null>) {
  ...
  useEffect(() => {
    if (active && ref.current) {
      ...
      if (elRect.top < bRect.top || elRect.bottom > bRect.bottom) {
        el.scrollIntoView({ block: 'nearest', inline: 'nearest' })
      }
    }
  }, [active, mode, ref])
