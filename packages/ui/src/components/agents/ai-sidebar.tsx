...
function ResourceRow({
  ...
  return (
    <motion.div
      ...
      draggable={reorderable && !row.item.disabled && !renaming}
      ...
      style={{
        paddingLeft: `${row.depth === 0 ? rootPaddingLeft : 12 + row.depth * 16}px`,
      }}
    >
...
export function AISidebar({
  ...
  const currentExpandedIds = useMemo(
    () => expandedIds ? new Set(expandedIds) : internalExpandedIds,
  ...
  const flat = useMemo(
    () => flattenResources(renderedItems, currentExpandedIds),
    [currentExpandedIds, renderedItems],
  );
  ...
  const moveCommands = useCallback(
    (row: FlatResource): SidebarResourceMoveCommands => {
      ...
      if (previous && canContain(previous.item) && previous.item.id !== parentId) {
        commands.into = {
          ...
          run: () => {
            const next = new Set(currentExpandedIds).add(previous.item.id);
            if (expandedIds === undefined) setInternalExpandedIds(next);
            onExpandedIdsChange?.(Array.from(next));
            void performMove({
              itemId: row.item.id,
    ...
    [currentExpandedIds, expandedIds, flat, onExpandedIdsChange, performMove],
  );
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent<HTMLDivElement>, row: FlatResource) => {
      ...
      if (moveModifier && event.key === "ArrowRight" && previous && canContain(previous.item)) {
        event.preventDefault();
        const nextExpanded = new Set(currentExpandedIds).add(previous.item.id);
        if (expandedIds === undefined) setInternalExpandedIds(nextExpanded);
      ...
      if (event.key === "ArrowRight" && canContain(row.item)) {
        event.preventDefault();
        if (!currentExpandedIds.has(row.item.id)) toggle(row.item.id);
  ...
  return (
    ...
    >
      <SharedLayoutBg
        className="gap-0.5"
        inset={0}
        pillClassName="rounded-none bg-hover"
      >
        {flat.map((row) => (
          <div key={row.item.id}>
            <ResourceRow
            ...
            expanded={currentExpandedIds.has(row.item.id)}
            focused={focusedRow === row.item.id}
            draggingId={draggingId}
            dropTarget={dropTarget}
            menuOpen={menuOpenId === row.item.id}
            moves={reorderable ? moveCommands(row) : {}}
            ...
            rootPaddingLeft={rootPaddingLeft}
